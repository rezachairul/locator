<x-layouts>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <div class="w-full overflow-x-auto">
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex item-center justify-center space-x-4 text-sm">
                                    <button id="updateProductButton-{{ $material->id }}" data-modal-target="updateProductModal-{{ $material->id }}" data-modal-toggle="updateProductModal-{{ $material->id }}" type="button" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                        </svg>
                                    </button>
                                    <!-- Modals Update -->
                                    <div id="updateProductModal-{{ $material->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative w-full max-w-2xl max-h-full">
                                            ...
                                            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-700">
                                                ...
                                                <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                                                    ...
                                                    <button type="button" data-modal-toggle="updateProductModal-{{ $material->id }}"
                                                ...
                                                <form action="{{route('material.update', $material->id)}}" method="post" enctype="multipart/form-data">
                                                    ...
                                                    <div class="mt-2 mb-4 sm:col-span-2">
                                                        <div>
                                                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-left">Jenis Material</label>
                                                            <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Jenis Material" required autofocus value="{{old('name', $material->name)}}">
                                                        </div>
                                                    </div>
                                    ...
                                    <button id="deleteButton-{{ $material->id }}" data-modal-target="deleteModal-{{ $material->id }}" data-modal-toggle="deleteModal-{{ $material->id }}" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete" type="button">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                ...
                <span class="flex items-center col-span-3">
                    Showing {{ $materials->firstItem() }} to {{ $materials->lastItem() }} out of {{ $materials->total() }}
                </span>
                ...
                <!-- Pagination -->
                 <div class="col-span-4 flex items-center justify-end space-x-2">
                    ...
                    @else
                        <a href="{{ $materials->previousPageUrl() }}" class="px-4 py-2 text-gray-500 bg-gray-200 rounded-md hover:bg-gray-300">Previous</a>
                    @endif
                    <span class="text-gray-300">Page {{ $materials->currentPage() }} of {{ $materials->lastPage() }}</span>
                    @if ($materials->hasMorePages())
                        <a href="{{ $materials->nextPageUrl() }}" class="px-4 py-2 text-gray-500 bg-gray-200 rounded-md hover:bg-gray-300">Next</a>
                    @else
        ...
        <!-- modals create -->
        @include('operasional.material.create')
    </div>
    <br>
</x-layouts>